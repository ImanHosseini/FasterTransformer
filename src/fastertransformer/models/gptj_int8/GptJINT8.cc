...
#include "src/fastertransformer/models/gptj_int8/GptJINT8.h"
...
template<typename T>
void GptJINT8<T>::initialize()
{
    gpt_context_decoder_ = new GptJContextDecoderINT8<T>(0,
                                                     0,
                                                     head_num_,
                                                     size_per_head_,
                                                     inter_size_,
    ...
    gpt_decoder_ = new GptJDecoderINT8<T>(0,
                                      head_num_,
                                      size_per_head_,
                                      inter_size_,
                                      num_layer_,
...
template<typename T>
void GptJINT8<T>::allocateBuffer()
{
    FT_CHECK(false);
}
...
template<typename T>
GptJINT8<T>::GptJINT8(size_t                              max_batch_size,
              size_t                              max_seq_len,
...
template<typename T>
GptJINT8<T>::GptJINT8(GptJINT8<T> const& gpt):
    BaseLayer(gpt),
...
template<typename T>
void GptJINT8<T>::registerCallback(callback_sig* fn, void* ctx)
{
...
template<typename T>
void GptJINT8<T>::unRegisterCallback()
{
...
template<typename T>
void GptJINT8<T>::forward(std::vector<Tensor>*       output_tensors,
                      const std::vector<Tensor>* input_tensors,
                      const GptJWeightINT8<T>*       gpt_weights)
{
    FT_CHECK(false);
}

template<typename T>
void GptJINT8<T>::forward(std::unordered_map<std::string, Tensor>*       output_tensors,
...
template<typename T>
void GptJINT8<T>::sendTensorsToFirstPipelineNode(std::unordered_map<std::string, Tensor>*       output_tensors,
                                             const std::unordered_map<std::string, Tensor>* input_tensors)
{
    if (pipeline_para_.world_size_ == 1) {
        // throw errors when detected
...
template<typename T>
void GptJINT8<T>::setOutputTensors(std::unordered_map<std::string, Tensor>*       output_tensors,
                               const std::unordered_map<std::string, Tensor>* input_tensors,
...
template<typename T>
size_t GptJINT8<T>::getPipelineParallelRank()
{
    return pipeline_para_.rank_;
}

template<typename T>
size_t GptJINT8<T>::getPipelineParallelSize()
{
    return pipeline_para_.world_size_;
}

template<typename T>
size_t GptJINT8<T>::getTensorParallelRank()
{
    return tensor_para_.rank_;
}

template<typename T>
size_t GptJINT8<T>::getTensorParallelSize()
{
    return tensor_para_.world_size_;
}

template<typename T>
bool* GptJINT8<T>::getFinishBuffer()
{
    return finished_buf_;
}

template class GptJINT8<float>;
