...
#include "src/fastertransformer/models/gptj_int8/GptJWeightINT8.h"

namespace fastertransformer {

template<typename T>
GptJWeightINT8<T>::GptJWeightINT8(const int                                  hidden_units,
    ...
    for (int l = 0; l < num_layer_; l++) {
        if (isValidLayerParallelId(l)) {
            decoder_layer_weights.push_back(
                GptJDecoderLayerWeightINT8<T>(hidden_units_, inter_size_, tensor_para_size_, tensor_para_rank_));
        }
        else {
            // Layer-parallelism: allocate empty layer because
            // this rank does not compute it:
            decoder_layer_weights.push_back(GptJDecoderLayerWeightINT8<T>(0, 0));
        }
    }
...
template<typename T>
GptJWeightINT8<T>::~GptJWeightINT8()
{
...
template<typename T>
GptJWeightINT8<T>::GptJWeightINT8(const GptJWeightINT8& other):
...
template<typename T>
GptJWeightINT8<T>& GptJWeightINT8<T>::operator=(const GptJWeightINT8& other)
{
    hidden_units_               = other.hidden_units_;
    inter_size_                 = other.inter_size_;
    vocab_size_                 = other.vocab_size_;
...
template<typename T>
void GptJWeightINT8<T>::setWeightPtr()
{
...
template<typename T>
void GptJWeightINT8<T>::mallocWeights()
{
...
template<typename T>
bool GptJWeightINT8<T>::isValidLayerParallelId(int l)
{
...
template struct GptJWeightINT8<float>;
template struct GptJWeightINT8<half>;
#ifdef ENABLE_BF16
template struct GptJWeightINT8<__nv_bfloat16>;
#endif
